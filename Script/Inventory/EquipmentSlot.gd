...
func insert_item(item):
	...
	var item_def = ItemDB.get_item(item.get_meta("id"))["ARMOR"]
	if item_slot != slot.name:
		return false
	if items[item_slot] != null:
		return false
	global.weapon_damage += item_damage
	global.pct_dam -= item_def
	print("damage= " + str(global.weapon_damage) + " and armor = " + str(global.pct_dam))
